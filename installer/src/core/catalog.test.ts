...
import type { SourceSkillIndexEntry } from "./types.js";

function createIndexedSkill(
  skill: Partial<SourceSkillIndexEntry> & Pick<SourceSkillIndexEntry, "name">,
...
test("catalog allow and deny policy filters results with deny precedence", () => {
  ...
  assert.deepEqual(
    searchCatalogSkills(skills, "service worker", { policy }).map((result) => result.skill.name),
    ["allowed-worker"],
  );
  assert.doesNotThrow(() => assertSkillAllowed("allowed-worker", "install", policy));
  assert.throws(
    () => assertSkillAllowed("blocked-worker", "use", policy),
    /denied for MCP use/,
  );
  assert.throws(
    () => assertSkillAllowed("missing", "install", { allowSkills: ["allowed-worker"] }),
