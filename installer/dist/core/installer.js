 ...
 */
export async function installSkill(platform, skill, config, srcDir) {
    const platformConfig = PLATFORM_CONFIGS[platform];
    const basePath = config.installType === "global"
    ...
    try {
        ...
        if (config.installCommands && platformConfig.supportsCommands) {
            const commandDir = config.installType === "global"
                ? expandPath(platformConfig.commandPath)
                : platformConfig.localCommandPath;
            const { cmdSrc, cmdExt } = getCommandSource(srcDir, skill.name, platformConfig.folderName);
            if (cmdSrc) {
                try {
                    await access(cmdSrc);
                    ...
                    if (platformConfig.folderName === "gemini" ||
 ...
 */
function getCommandSource(srcDir, skillName, platformFolder) {
 ...
 */
export async function updateGitignore(platforms, config) {
 ...
 */
export async function executeInstallation(config, skills, srcDir) {
    const results = [];
