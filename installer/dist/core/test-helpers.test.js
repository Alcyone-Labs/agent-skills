...
export async function createSkill(skillsRoot, skillName, options = {}) {
    const skillDir = join(skillsRoot, skillName);
    const binDir = join(skillDir, "bin");
    const commandNames = options.commandNames ?? [`${skillName}-cmd`];
    const skillMarkdown = options.skillMarkdown ??
        (options.description
    ...
    for (const commandName of commandNames) {
        ...
        const commandContent = options.commandContents?.[commandName] ?? "#!/usr/bin/env bash\nexit 0\n";
