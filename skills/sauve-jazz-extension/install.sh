...
REPO_URL="https://github.com/Alcyone-Labs/skill-forge.git"
...
main() {
  ...
  install_skill_to() {
    ...
    if [[ -n "$command_dir" ]]; then
      ...
      if [[ "$p_norm" == "opencode" ]]; then
        ...
        cmd_src="${cmd_base}/droid/${skill_name}.md"
        cmd_ext=".md"
      fi
  ...
  echo ""
  echo "Done! You can now use the skill with:"
  echo "  OpenCode: /sauve-jazz-extension help me add a feature"
  echo "  Gemini:   gemini -c sauve-jazz-extension 'Debug sync issue'"
}
